...
def plot_kagome(J_value, stars, base_path, output_path):
    ...
    adjusted_df = pd.DataFrame({'Site': adjusted_sites, 'LocalSpin': local_density})
    vertices = adjusted_df['Site']
    local_density_ad = adjusted_df['LocalSpin']
    ...
    for point, index in zip(points, range(1, len(points) + 1)):
        if index in vertices.values:
            ...
            color_index = 1 if diameter_size >= 0 else 0
            plt.scatter(point[0], point[1], color=colors[color_index], marker='o', s=abs(diameter_size) * 4000, zorder=2) #2000 für 11stars mit 500 für J=0.0 und 4000 für 5

    plt.gca().set_aspect('equal', adjustable='box')
    plt.axis('off')
    plt.savefig(f'{output_path}/5stars_local_spin_S1_{J_value}.pgf', bbox_inches='tight', pad_inches=0.2)
    plt.close()
...
def main():
    ...
    base_path = ''#speficy
    output_path = '' #specify
    J_values = np.arange(0.0, 3.0, 0.5)
